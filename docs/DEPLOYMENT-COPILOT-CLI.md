...
> The Azure storage tables (`messagetemplates`, `messagebatches`, `messagelogs`, `pendingdeliveries`, `ConversationCache`, `usercache`, `usersyncmetadata`, `smartgroups`, `smartgroupmembers`, `appsettings`), the blob container `message-templates`, and the queues `batch-messages` and `batch-control` are auto-created by the application on first run — no need to provision them manually.
